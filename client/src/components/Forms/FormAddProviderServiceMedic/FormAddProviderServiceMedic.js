...
export const FormAddProviderServiceMedic = ({user, resetPage, setResetPage, showProviderService, handleCloseProviderService, handleShowProviderService, listProvinces}) => {
    ...
    const getCity = (selectedProvince) => {
        ...
        axios
            ...
            .then((res) => {
                ...
                setProviderService({ ...providerService, province_id: selectedProvince});
            })
            .catch((error) => {
                console.log(error);
            });
    ...
    const onSubmit = () => {
        ...
        if(providerService.province_id !== null && providerService.city_id !== null){
            setMessageError("");
            axios
            .post(`http://localhost:4000/medic/providerServices/${user?.user_id}`, providerService)
            .then((res) => {
            ...
            .catch((err) => {
                ...
                {
                    setMessageError("Ya tienes agregado un servicio para la Ciudad indicada");
                }
                else{
                    console.log(err);
                }
    ...
    <Modal show={showProviderService} onHide={handleCloseProviderService}>
        ...
        <Modal.Body>
            <InputGroup className='mb-3'>
              <Form.Select onChange={(e) => getCity(e.target.value)} name="province_id" aria-label="Default select example">
            ...
            <InputGroup className='mb-3'>
              ...
              <Form.Select name="city_id" onChange={handleChange} aria-label="Default select example">
